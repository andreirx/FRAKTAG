...
This repo can be indexed by `rgr` (Repo-Graph). Index the engine package:

```bash
rgr repo add packages/engine --name fraktag
rgr repo index fraktag                   # ~53 files, <200ms
rgr graph cycles fraktag                 # Known cycles: core<->prompts, core<->nuggets
rgr graph dead fraktag --kind SYMBOL     # Unreferenced exports
rgr graph callers fraktag <symbol> --edge-types CALLS,INSTANTIATES
```

Use `/investigate-symbol fraktag <SymbolName>` for guided analysis. Use `/repo-overview packages/engine` for structural health.
